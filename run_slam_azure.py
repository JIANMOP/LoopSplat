...
# Set Qt environment variables before any Qt-related imports to avoid GUI initialization
...
from src.entities.datasets_azure import get_azure_dataset

# We need to patch before importing GaussianSLAM
import src.entities.datasets as datasets
original_get_dataset = datasets.get_dataset

def patched_get_dataset(dataset_name: str):
    if dataset_name == "azure_kinect":
        return get_azure_dataset(dataset_name)
    return original_get_dataset(dataset_name)

# Apply the patch
datasets.get_dataset = patched_get_dataset

# Also patch the evaluator's get_dataset import
import src.evaluation.evaluator as evaluator_module
evaluator_module.get_dataset = patched_get_dataset

# Now import GaussianSLAM after patching
from src.entities.gaussian_slam import GaussianSLAM


def get_args():
    parser = argparse.ArgumentParser(
        description='Arguments to run Azure Kinect SLAM')
