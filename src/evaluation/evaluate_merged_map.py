...
def refine_global_map(pt_cloud: o3d.geometry.PointCloud, training_frames: list, max_iterations: int,
    ...
    if pt_cloud is None:
        ...
        if not output_mesh.exists():
            print(f"Warning: mesh not found at {output_mesh}, "
                  "returning empty Gaussian model")
            return gaussian_model
        output_mesh = o3d.io.read_triangle_mesh(str(output_mesh))
        if len(output_mesh.vertices) == 0:
            print("Warning: mesh has 0 vertices, returning empty Gaussian model")
            return gaussian_model
