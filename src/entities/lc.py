...
class Loop_closure(object):
    ...
    def has_valid_gt_poses(self):
        """Check if the dataset has valid ground truth poses (not dummy poses)

        Returns:
            bool: True if valid GT poses exist, False if using dummy poses
        """
        if not self.cam_dict:
            return False

        # Get a sample of cameras to check
        sample_cams = list(self.cam_dict.values())[:min(3, len(self.cam_dict))]

        # Check if all GT poses are identical (indicating dummy poses)
        first_cam = sample_cams[0]
        first_R_gt = first_cam.R_gt
        first_T_gt = first_cam.T_gt

        for cam in sample_cams[1:]:
            # If any pose differs significantly, we have valid GT
            if not torch.allclose(cam.R_gt, first_R_gt, atol=1e-6) or \
               not torch.allclose(cam.T_gt, first_T_gt, atol=1e-6):
                return True

        # If all poses are identical, check if they're identity/zero (dummy poses)
        identity_R = torch.eye(3, device=first_R_gt.device, dtype=first_R_gt.dtype)
        zero_T = torch.zeros(3, device=first_T_gt.device, dtype=first_T_gt.dtype)

        # If GT poses are identity rotation and zero translation, they're dummy poses
        if torch.allclose(first_R_gt, identity_R, atol=1e-6) and \
           torch.allclose(first_T_gt, zero_T, atol=1e-6):
            return False
    ...
    def analyse_pgo(self, odometry_edges, loop_edges, pose_graph):
        ...
        # Check if we have valid ground truth poses for ATE evaluation
        if self.has_valid_gt_poses():
            print("Evaluating ATE before pgo ...")
            eval_ate(self.cam_dict, self.kf_ids, str(pgo_save_path/"before"), 100, final=True)
        else:
            print("Skipping ATE evaluation before pgo - no valid ground truth poses available")
        ...
        if self.has_valid_gt_poses():
            ...
            print("Skipping ATE evaluation after pgo - no valid ground truth poses available")
        ...
        if self.has_valid_gt_poses():
            ...
            print("Skipping ATE evaluation within submaps - no valid ground truth poses available")
